...
object OcrRecipeParser {
    ...
    private val TYPO_SHADOVW_RE    = Regex("""(?i)\bshadovw\b""")
    private val TYPO_REUGHNESS_RE  = Regex("""(?i)\breughness\b""")
    private val ZERO_FF_RE         = Regex("""(?i)\b0FF\b""")
    ...
    private fun parseInternal(rawText: String, geomStitched: String?): OcrParseResult? {
        ...
        val normalizedText = rawText
            ...
            .replace(TYPO_SHADOVW_RE, "shadow")
            .replace(ZERO_FF_RE, "OFF")
    ...
    private val NR_ABBREV_RE = Regex("""(?i)\bNR[ \t]*([+-]?\d+(?:[.,]\d+)?)""")
    ...
    private val WB_TEMP_RE = Regex("""(?i)([\dO]{4,5})\s*[Ok0Kk]\b""")
    ...
    private fun parseWhiteBalanceWithShifts(text: String): Triple<String?, String?, String?> {
        ...
        if (wbDisplay == null) {
            WB_TEMP_RE.find(text)?.let { m ->
                val sanitized = m.groupValues[1].replace('O', '0').replace('o', '0')
                val kelvin = sanitized.toIntOrNull()
                if (kelvin != null && kelvin in 2500..10000) {
                    wbDisplay = "${sanitized}K"
    ...
    private val COL_WB_RE     = Regex("""(?i)(auto|daylight|shade|incandescent|underwater|fluorescent|[\dO]{4,5}\s*[kK])""")
