...
object FujiHaptics {
    ...
    fun perform(
        ...
        when (effect) {
            ...
            FujiHapticEffect.SearchTap -> {
                return primitiveSequence(
                    context = context,
                    steps = listOf(
                        FujiHapticPrimitiveStep(VibrationEffect.Composition.PRIMITIVE_CLICK, 0.88f),
                    ),
                    fallbackLabel = "search tap",
                    fallbackEffectId = VibrationEffect.EFFECT_CLICK,
                )
            }

            FujiHapticEffect.SearchRunning -> {
                // Gentle double-pulse scan pattern, repeating — caller must cancel()
                return waveformPatternRepeating(
                    context = context,
                    timings = longArrayOf(0, 14, 110, 10, 260),
                    amplitudes = intArrayOf(0, 100, 0, 65, 0),
                    repeatIndex = 0,
    ...
    private fun waveformPatternRepeating(
        context: Context,
        timings: LongArray,
        amplitudes: IntArray,
        repeatIndex: Int,
        ...
        vibrator.vibrate(effect)
        return FujiHapticResult(true, "$label: repeating waveform")
    }
